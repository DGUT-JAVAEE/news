	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(type.equals("register")){
			...
			if(result==1){
				...
				message.setRedirectUrl(request.getContextPath() + "/user/free/login.jsp");
			}else if(result==0){
				message.setMessage("同名用户已存在，请改名重新注册！");
				message.setRedirectUrl(request.getContextPath() + "/user/free/register.jsp");
			}else{
			...
			if(result==1){
				...
				if(originalUrl==null)
					response.sendRedirect(request.getContextPath() + "/user/manageUIMain/manageMain.jsp");
				else
			...
			if(users==null){
				...
				message.setRedirectUrl(request.getContextPath() + "/servlet/UserServlet?type1=check&page=1&pageSize=2");
			}else{
				request.setAttribute("pageInformation", pageInformation);
				request.setAttribute("users", users);
			...
			if(result==5){
				...
				message.setRedirectUrl(request.getContextPath() + "/servlet/UserServlet?type1=showPrivate");
			}else if(result==0){
