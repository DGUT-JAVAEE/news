	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(type.equals("add")){
			...
			if(result==1){
				...
				message.setRedirectUrl(request.getContextPath() + "/news/manage/addNews.jsp");
			}else if(result==0){
				message.setMessage("添加新闻失败！请联系管理员！");
				message.setRedirectUrl(request.getContextPath() + "/index.jsp");
			}
			...
			if("all".equals(newsType)) {
				pageInformation.setSearchSql(" 1=1 ");
			}else {
			...
			pageInformation.setSearchSql(" caption like'%"+search+"%' ");
			List<List<String>> newsCaptionsList=new ArrayList<List<String>>();
			List<News> newss=newsService.getOnePage(pageInformation);
			request.setAttribute("newsesList", newss);
			request.setAttribute("newsCaptionsList",newsCaptionsList);
			request.setAttribute("homePageNewsCaptionMaxLength", 
					Integer.parseInt(WebProperties.config.getString("homePageNewsCaptionMaxLength")));
			getServletContext().getRequestDispatcher("/index4.jsp").include(request,response);
			return;
		}
	}
