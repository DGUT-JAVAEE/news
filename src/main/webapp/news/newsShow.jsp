...
<html>
 <head>
  ...
  <body>
  	<form action="${pageContext.request.contextPath }/newsServlet?type1=showNews" id="myform" method="post">
  	 <table align="center" border='1' class="table">
	    ...
	    <c:forEach items="${requestScope.newses}"  var="news">      
	   		<tr>
		   		<td>${news.newsId}</td>     
		   		<td><a href="${pageContext.request.contextPath }/newsServlet?type1=showANews&newsId=${news.newsId}&page=1&pageSize=2">${news.caption}</a></td>	
		   		<td>${news.author}</td>     
		   		<td><myTag:TimestampTag dateTime="${news.newsTime}" type="YMDHMS-"/></td>	
		   	</tr>
		</c:forEach> 	    
	</table>
