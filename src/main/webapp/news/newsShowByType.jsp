...
<html>
 <head>
  ...
  <body><div class="center" style="width:810px"> 
  	<form action="${pageContext.request.contextPath }/newsServlet?type1=showNewsByNewsType&newsType=${requestScope.newsType}" id="myform" method="post">
  	  ...
  	  <div class="newsShowByType_frame center" id="frameDiv">
  	  	<div class="newsShowByType_left" id="leftDiv">
				...
				<c:choose>
				    ...
				    <c:otherwise>
						<li><a href="${pageContext.request.contextPath }/newsServlet?type1=showNewsByNewsType&newsType=all&page=1&pageSize=5">
  	  					全部</a></li>	
				    </c:otherwise>				        
				</c:choose>	  	  			
	 			...
	 			<c:forEach items="${requestScope.newsTypes}"  var="newsType1">	  	 			
					<c:choose>
					    ...
					    <c:otherwise>
					    	<li><a href="${pageContext.request.contextPath }/newsServlet?type1=showNewsByNewsType&newsType=${newsType1.name}&page=1&pageSize=5">
					    		${newsType1.name}</a></li>
					    </c:otherwise>				        
					</c:choose>				
				</c:forEach> 	  		
		...
		<div class="newsShowByType_rightTop">
			<div><ul>
					<c:forEach items="${requestScope.newses}"  var="news"> 
						<li><a href="${pageContext.request.contextPath }/newsServlet?type1=showANews&newsId=${news.newsId}&page=1&pageSize=2" target="_blank">${news.caption}</a></li>
					</c:forEach>
				</ul>
			</div>	
		</div>
